...
\begin{equation} s_{\beta} = \frac{s_{maxhalfCI}}{\sqrt{var(x)}} \label{eq:uni1} \end{equation}
...
where the $s_{meanhalfCI}$ evd based residuals variance in the slope related term of \eqref{eq:uni2}, provides better coverage performance for the evd based intercept CI estimator based on recent (linear expanding heteroscedasticity) results (4). 
...
\begin{equation} s_{\beta_1} = \frac{s_{maxhalfCI}}{\sqrt{var(x_1)(1-r^2_{12})}} \label{eq:b1} \end{equation}

\begin{equation} s_{\beta_2} = \frac{s_{maxhalfCI}}{\sqrt{var(x_2)(1-r^2_{12})}} \label{eq:b2} \end{equation}
...
+ \ var(\hat{\beta_1})median(x_1)^2 + var(\hat{\beta_2})median(x_2)^2 + cov(\hat{\beta_1},\hat{\beta_2})median(x_1)median(x_2) \\
\approx \ var(median(y)) + var(\hat{\beta_1})median(x_1)^2 + var(\hat{\beta_2})median(x_2)^2 
 + \ cov(\hat{\beta_1},\hat{\beta_2})median(x_1)median(x_2)  \label{eq:var}
\end{multline} 
