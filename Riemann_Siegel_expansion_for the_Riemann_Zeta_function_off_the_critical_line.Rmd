...
The calculations in this paper involving zeta and its numerical derivatives used the "pracma" r package (6).
...
\begin{equation}
abs(\zeta(s)) = \sqrt{Z(t)^2+\alpha^2Z^\prime(t)^2+\beta^2Z^{\prime\prime}(t) + ....}   \label{eq:serexp}
\end{equation}
...
\begin{equation}
abs(\zeta(s)) \approx (1) \cdot \sqrt{Z(t)^2+abs(Re(s)-0.5)^2Z^\prime(t)^2+ ....}   \qquad for \thinspace Re(s) \thinspace < \thinspace 0.5 \label{eq:1storder}
\end{equation}
...
***Figure 2: First order Riemann Siegel series expansion and Riemann Zeta function behaviour, below the critical line for s = (0.2 + It)***
...
\begin{equation}
abs(\zeta(s)) \approx A \cdot \sqrt{Z(t)^2+B^2Z^\prime(t)^2+ B^3Z^{\prime\prime}(t)^2 + B^4Z^{\prime\prime\prime}(t)^2+....}   \qquad for \thinspace Re(s) \thinspace < \thinspace 0.5 \label{eq:4thorder}
\end{equation}

where 

\begin{equation}
A = abs(2^s\pi^{(s-1)}sin(\frac{s}{2}\pi)\Gamma(1-s))
\end{equation}
