...
author: "John Martin" 
date: created "Wed, April 18th, 2018" last modified "Wed, April 25th, 2018"
output:
  pdf_document:
    fig_crop: no
...
***Figure 4. Change in height of large peak of $\zeta(s)$ by examining the partial Euler Product Riemann Siegel Z function for different numbers of contributing primes along (a) s=(1+$i*t$) and (b) s=(0.5+$i*t$) near the first Rosser rule violation.***
...
plot(x=df[,1],y=abs(df[,2]+1i*df[,3]),col=1,lwd=2,typ="l",xlab="imag part s=(1+it) line",ylab="abs(function) value",xlim=c( 445575490,445575492.5),ylim=c(0,10))
...
***Figure 5. Change in height of large peak of $\zeta(s)$ by examining the partial Euler Product Riemann Siegel Z function for different numbers of contributing primes along (a) s=(1+$i*t$) and (b) s=(0.5+$i*t$) near t=445,575,490.***

Further work is needed to properly understand the progression in number of significantly contributing primes to the signal strength as $t \rightarrow \infty$. However, the technique does continue to work well at the highest t, when comparing to known $\zeta(s)$ data (1,2, 16, 17). For example, Figure 6 shows the partial Euler Product extended Riemann Siegel Z function behaviour at 39246764589894309155251169284104.0506 (~3.29e31) which has a Z(t) peak ~16244.8652, where the $Z_{extEP}(s,hybrid)$ calculations were performed using pari/gp software (18). The first order shifts in the riemann zeta zeroes on the critical line are well approximated (pink line) using only the first 1000 primes, with again the accompanying s=1 peak  being well described by the first 50 primes. In contrast, $\Im(log(\zeta_{EP}(s)))$ using the same first 1000 primes (black line) has badly lost resolution with only 4 discontinuities probably related to the Rosser rule violations near the peak.  


```{r, fig6, echo=FALSE, cache=TRUE, fig.width=7, fig.height=5,  fig.keep='high', warning=FALSE }


par(mfrow=c(1,2))

 xsf <- seq(1/2000.,2,l=4000)



df2 <- read_fn(dir_loc,"large_T_1_Z")
df2[,"V1"] <- as.numeric(gsub("[ ]", "", df2[,1]))
df2[,"V2"] <- as.numeric(gsub("[ ]", "", df2[,2]))
df2[,"V3"] <- as.numeric(gsub("[ ]", "", df2[,3]))

df3 <- read_fn(dir_loc,"large_T_1_Z_1")
df3[,"V1"] <- as.numeric(gsub("[ ]", "", df3[,1]))
df3[,"V2"] <- as.numeric(gsub("[ ]", "", df3[,2]))
df3[,"V3"] <- as.numeric(gsub("[ ]", "", df3[,3]))

df4 <- read_fn(dir_loc,"large_T_1_Z_4")
df4[,"V1"] <- as.numeric(gsub("[ ]", "", df4[,1]))
df4[,"V2"] <- as.numeric(gsub("[ ]", "", df4[,2]))
df4[,"V3"] <- as.numeric(gsub("[ ]", "", df4[,3]))

df50 <- read_fn(dir_loc,"large_T_1_Z_50")
df50[,"V1"] <- as.numeric(gsub("[ ]", "", df50[,1]))
df50[,"V2"] <- as.numeric(gsub("[ ]", "", df50[,2]))
df50[,"V3"] <- as.numeric(gsub("[ ]", "", df50[,3]))


dfy <- read.csv(paste0(dir_loc,"large_T_zetazeroes.csv"),sep=",",header=TRUE,skip=1)
dfy[,1] <- as.numeric(as.character(dfy[,1]))

plot(x=xsf,y=abs(df2[,1]+1i*df2[,2]),col=1,lwd=2,typ="l",xlab="imag part s=(1+it) line ~3.92e31",ylab="abs(function) value",ylim=c(0,10))
lines(x=xsf,y=df2[,1],col=2,lwd=2,lty=6)
 # abline(h=0,lty=3); abline(v=dfy[,2],lty=3)
lines(x=xsf,y=df3[,1],col=5,lty=6,lwd=2);
lines(x=xsf,y=df4[,1],col=4,lty=6,lwd=2);
lines(x=xsf,y=df50[,1],col="brown",lty=6,lwd=2);

legend("topleft", legend = c("|Re(Z_ext_first_prime)|","|Re(Z_ext_4_primes)|","|Re(Z_ext_50_primes)|","|Re(Z_ext_1000_primes)|","|Z_ext_1000_primes|"),
  col = c("cyan","blue","brown","red","black"),  pch = -1, lty = c(6,6,6,2,1), lwd = 2, cex = 0.6)




df2 <- read_fn(dir_loc,"large_T_0_5_Z")
df2[,"V1"] <- as.numeric(gsub("[ ]", "", df2[,1]))
df2[,"V2"] <- as.numeric(gsub("[ ]", "", df2[,2]))
df2[,"V3"] <- as.numeric(gsub("[ ]", "", df2[,3]))

df3 <- read_fn(dir_loc,"large_T_0_5_Z_1")
df3[,"V1"] <- as.numeric(gsub("[ ]", "", df3[,1]))
df3[,"V2"] <- as.numeric(gsub("[ ]", "", df3[,2]))
df3[,"V3"] <- as.numeric(gsub("[ ]", "", df3[,3]))

df4 <- read_fn(dir_loc,"large_T_0_5_Z_4")
df4[,"V1"] <- as.numeric(gsub("[ ]", "", df4[,1]))
df4[,"V2"] <- as.numeric(gsub("[ ]", "", df4[,2]))
df4[,"V3"] <- as.numeric(gsub("[ ]", "", df4[,3]))

df50 <- read_fn(dir_loc,"large_T_0_5_Z_50")
df50[,"V1"] <- as.numeric(gsub("[ ]", "", df50[,1]))
df50[,"V2"] <- as.numeric(gsub("[ ]", "", df50[,2]))
df50[,"V3"] <- as.numeric(gsub("[ ]", "", df50[,3]))

df <- read_fn(dir_loc,"large_T_0_5_EP")
df[,"V1"] <- as.numeric(gsub("[ ]", "", df[,1]))
df[,"V2"] <- as.numeric(gsub("[ ]", "", df[,2]))
df[,"V3"] <- as.numeric(gsub("[ ]", "", df[,3]))
...
plot(x=xsf,y=(df2[,1]),col=2,lwd=2,typ="l",xlab="imag part s=(0.5+it) line ~3.92e31",ylab="abs(function) value",ylim=c(-pi,20))
lines(x=xsf,y=df2[,3],col=6,lwd=1)
abline(h=0,lty=3); abline(v=dfy[,1]-19,lty=3)
lines(x=xsf,y=df3[,1],col=5,lty=6,lwd=2);
lines(x=xsf,y=df4[,1],col=4,lty=6,lwd=2);
lines(x=xsf,y=df50[,1],col="brown",lty=6,lwd=2);
lines(x=xsf,y=df[,3],col=1,lwd=2);

legend("topleft", legend = c("|Re(Z_ext_first_prime)|","|Re(Z_ext_4_prime)|","|Re(Z_ext_50_primes)|","|Re(Z_ext_1000_primes)|","zeta(s) zeroes","imag(log(Z_ext_1000_primes))","imag(log(zetaEP(s)))"),
  col = c("cyan","blue","brown","red","black","magenta","black" ),  pch = -1, lty = c(6,6,6,1,3,1,1), lwd = 2, cex = 0.6)


```

***Figure 6. Change in height of large peak of $\zeta(s)$ by examining the partial Euler Product Riemann Siegel Z function for different numbers of contributing primes along (a) s=(1+$i*t$) and (b) s=(0.5+$i*t$) near t=3.9246764589894309155251169284103e31.***

\newpage 

###Constructing a $-\pi S(t)$ analogue for $\Im(log(Z_{extEP}(s,hybrid)))$ to determine if Rosser rule violations can be identified via $Z_{extEP}(s,hybrid)$

Figure 7 below illustrates the continuous version of $\Im(log(Z_{extEP}(s,hybrid)))$ and the aggregated phase increments from the branch points of $\Im(log(Z_{extEP}(s,hybrid)))$ in comparison to the continuous version of $\theta(t)+\zeta(0.5+it)$ near the first Rosser rule violation point ~6,820,050.

At each branch point of $\Im(log(Z_{extEP}(s,hybrid)))$, a $\pi$ phase increment (4) can be added to the discontinuous $\Im(log(Z_{extEP}(s,hybrid)))$ function bounded by $\pm\frac{pi}{2}$ to create the continuous version of $\Im(log(Z_{extEP}(s,hybrid)))$. The overall trend of the continuous function on the critical line is $\theta(t)$ which is also, of course, the trend behaviour of $\zeta(0.5+it)$. In figure 7, instead of adding all the phase increments from t=14.137 onwards, $\theta(6,820,048)+\pi/2$ was used as a shortcut to establish the correct baseline of the continuous function between 6,820,048 \& 6820,052.5.

Looking at the intersection of the (blue) $\pm\pi, -2\pi$ bounds on $\theta(t)$, with the (green) aggregated phase increments of $\Im(log(Z_{extEP}(s,hybrid)))$ it can be seen that Gram points violations and Rosser rule violations can be observed close to these intersection points at $\pm\pi, \pm2\pi$ respectively. Thus through careful mapping, approximate identification of Rosser rule violations may generally be possible using $\Im(log(Z_{extEP}(s,hybrid)))$.




```{r, fig7, echo=FALSE, cache=TRUE, fig.width=7, fig.height=5,  fig.keep='high', warning=FALSE }
...
zphase <- add_logphase(xse,df2$V6)
zetaphase <- add_logphase(xse,Im(log(sqrt((df$V2+1i*df$V3)*(df$V2-1i*df$V3)))))

plot(x=xse,y=fug(xse[1])+df2$V6+zphase+pi/2,typ="l",
     ylim=c(min(fug(xse[1])+df2$V6+zphase+pi/2),max(fug(xse[1])+df2$V6+zphase+pi/2)+40),
     lwd=2,xlab="imag part",ylab="function value")
lines(x=xse,y=fug(xse),col=2)
lines(x=xse,y=fug(xse)+pi,col=4,lty=2)
lines(x=xse,y=fug(xse)-pi,col=4,lty=2)
lines(x=xse,y=fug(xse)-2*pi,col=4,lty=2)
lines(x=xse,y=fug(xse[1])+zphase+pi/2,col=3)
lines(x=xse,y=fug(xse)+exp(1i*zphase)*abs(df$V5+1i*df$V6),col=5)
abline(v=dfy[c(11,13,14),2],lty=3)

legend("topleft", legend = c("continuous version imag(log(Z_ext_1000_primes))","theta(t)","theta(t)+pi, -pi, -2*pi bounds on distance from theta(t)","theta(t) + imag(log(Z_ext_1000_primes)) branch point phase increments"," theta(t) + zeta(t) \n (using imag(log(Z_ext_1000_primes)) branch points for phase changes)","Gram rule & Rosser rule violations"),
  col = c("black","red","blue","green","cyan","black" ),  pch = -1, lty = c(1,1,2,1,1,3), lwd = 2, cex = 0.6)

```

***Figure 7. Constructing an estimate of $-\pi S(t)$ from the continuous version of $\Im(log(Z_{extEP}(s,hybrid))) + Z_{extEP}(s,hybrid)$ with comparison to $\theta(t)$ and $\theta(t) + \zeta(s)$. ***
...
Through careful mapping of the aggregated phase increments arising from the branch points of $\Im(log(Z_{extEP}(s,hybrid)))$ it is also possible to approximately identify Gram and Rosser rule violation points. 
...
16. Bober, J.W., Hiary, G.A. (2016) New computations of the Riemann Zeta function on the critical line. Exp. Math., 27, 1–13

17. Tihanyi, N., Kovács, A. & Kovács, J. "Computing Extremely Large Values of the Riemann Zeta Function" J Grid Computing (2017) 15: 527. https://doi.org/10.1007/s10723-017-9416-0

18. The PARI~Group, PARI/GP version {\tt 2.9.4}, Univ. Bordeaux, 2018,
...
##Appendix B: Basic pari/gp code used to calculate $\zeta_{EP}(s)$, $Z_{extEP}(s)$ etc
...
res = 0.5
pr_lim = 7920
bp = 39246764589894309155251169284103.
forstep(X=1,4000,1,listput(s1,X/step))
v1=Vec(s1)
s2=listcreate(4000)

forstep(X=1,4000,1,[s2=sqrt(prodeuler(p=2,pr_lim,1/(1-1/p^(res+I*(bp+X/step))))^2
/exp(log(2^(res+I*(bp+X/step))*Pi^((res+I*(bp+X/step))-1))+
(Pi/2*(bp+X/step)-log(2)+I/2*Pi*(1-res))+lngamma((1-(res+I*(bp+X/step)))))*
abs(exp(log(2^(res+I*(bp+X/step))*Pi^((res+I*(bp+X/step))-1))+
(Pi/2*(bp+X/step)-log(2)+I/2*Pi*(1-res))+lngamma((1-(res+I*(bp+X/step))))))),
write(large_T_0_5_Z,real(s2),",",imag(s2),",",imag(log(s2)))])
s3=listcreate(4000)
forstep(X=1,4000,1,[s3=prodeuler(p=2,pr_lim,1/(1-1/p^(res+I*(bp+X/step)))),
write(large_T_0_5_EP,real(s3),",",imag(s3),",",imag(log(s3)))])
