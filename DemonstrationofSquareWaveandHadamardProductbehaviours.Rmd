...
In this paper, $\frac{Z_{ext}(s)^2}{\zeta(s)}$ is used to create a square wave Riemann Zeta generating function, which highlights and identifies the odd positive integers and prime numbers. 

##Square wave version of Riemann Zeta generating function

Using eqn \eqref{eq:zext} it can be shown that

\begin{align}
\frac{Z_{ext}(s)^2}{\zeta(s)} &= \zeta(1-s)*abs(2^s\pi^{s-1}sin(\frac{\pi s}{2})\Gamma(1-s)) \label{eq:zetasq}
\end{align}
...
This square wave behaviour allows the positive odd integers to be identified without recourse to a arbitrarily defining a staircase function. That is, the function values for the real numbers between the odd positive integers now have an independent function defining their values.

This function when extended with a quotient argument s/n where n the denominator is the integer,

\begin{align}
\frac{Z_{ext}(\frac{s}{n})^2}{\zeta(\frac{s}{n})} &= \zeta(1-\frac{s}{n})*abs(2^\frac{s}{n}\pi^{\frac{s}{n}-1}sin(\frac{\pi \frac{s}{n}}{2})\Gamma(1-\frac{s}{n})), \qquad \text{where } n \in \mathbb{N}  \label{eq:qsq}
\end{align}
...
The ratio $\frac{Z_{ext}(\frac{s}{n})^2}{\zeta(\frac{s}{n})}$ can be used to identify the positive odd integers and prime numbers. This function which allows input values off the positive real axis may give useful insights to the prime-counting function. 
