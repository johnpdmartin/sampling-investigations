...
```
***Figure 5. $P_{BIC}(D|H0)$ as a function of p value for different sample sizes under simple RCT analysis when $P(H0)=0$,$\alpha = 0.05$ and the statistical power is ~95%. In this simulation the effect size is different for each fixed sample size to achieve ~95% and for clarity only 100 repeated samples are shown. The underlying contour lines under the data points were obtained from figure 1. ***
