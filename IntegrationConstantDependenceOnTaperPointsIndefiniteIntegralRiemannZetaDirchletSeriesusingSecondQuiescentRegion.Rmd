...
\begin{equation}
\textbf{C}_{\left[\int \zeta(s)\zeta(1-s) ds\right]_{\Im(s) \rightarrow \infty}} \sim \beta \cdot (2p)^\alpha \label {eq:integration_constant_model}
\end{equation}
...
y2k=c(13824.831316181456209,13824.0815828820290015,13821.5556619668284606,13816.5326480883895225272,13798.19357447256755,NA,NA)
y13k=c(114318.3683530846,114317.6186199787,114315.0926990635317,114310.06968518509276,114291.73061156927080,114239.87553071330999,NA)
y26k=c(246617.089167129934,246616.339434019991,246613.8135131047808,246608.790499226341954,246590.451425610519988,246538.59634475455918,246505.371044607229888)
y29986.206=c(288595.2811671223760,288594.531434013251,288592.005513098040,288586.982499219601779,288568.6434256037798,288516.788344747819,288483.5630446004897)
...
knitr::kable(df)
```
***Table 1:End tapered Riemann Zeta Dirichlet Series approximations of $\\$ $\Im\left[\int\zeta(1/2+I*t)\zeta(1-(1/2+I*t)) ds\right]_{\Im(s) \rightarrow \infty}$ for $\Im(s)=\{13000,26000,29986.206\}$***
...
plot(xt2,yt,xlim=range(c(0,xt2)),ylim=range(c(yt,yt+10)),
     main="differences in indefinite integral as function of 2p \n when alpha =0.5",ylab="indefinite integral at s=1/2+I*29986.206",xlab="2p^alpha")
grid()
...
plot(xt2,yt,xlim=range(c(0,xt2)),ylim=range(c(yt,yt+10)),
     main="differences in indefinite integral as function of 2p \n when alpha =0.49934",ylab="indefinite integral at s=1/2+I*29986.206",xlab="2p^alpha")
grid()
abline(lm(yt~xt2),col=2)
...
Secondly, Figure 5 shows what happens if the nuisance discontinuities in the harmonic series $\sum_{k=1}^{\lfloor\frac{t}{\pi}\rfloor}\frac{1}{k}$ ${\color{green}green}$ are removed, i.e, comparing $\left(t\cdot\sum_{k=1}^{\lfloor\frac{t}{\pi}\rfloor}\frac{1}{k}\right) - \lfloor\frac{t}{\pi}\rfloor\cdot \pi$ which is higher in magnitude than $t*\log(t) -t*(1+\log(2\pi)-2\gamma)$ and as $\Im(s) \rightarrow \infty$ grows at a higher rate than the second moment indefinite integral. So the imaginary components of the second, third etc terms in equation (7) are increasingly cancel out with the first term.
