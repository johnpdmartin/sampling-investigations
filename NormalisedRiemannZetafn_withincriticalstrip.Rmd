---
title: Exact functional dependence for the growth in the magnitude of the Riemann Zeta function within the critical strip. 
...
The growth of the magnitude in the Riemann Zeta function has the exact dependence $\zeta_{growth factor}(s)=abs\Big(e^{(-abs(\Re(s)-\frac{1}{2})+\Im(s)\frac{\pi}{4}+\frac{\gamma}{4\pi})}\pi^{-\frac{s}{2}}\Gamma(\frac{s}{2})\Big)$ for $0 \le \Re(s) \le 1$, ie. within the critical strip. The growth factor $\zeta_{growth factor}(s)$ is $< log(\Im(s))$ in agreement with the Lindelof hypothesis. This result arises naturally from the normalised Riemann Zeta function $\zeta_{norm}(s)=e^{(-abs(\Re(s)-\frac{1}{2})+\Im(s)\frac{\pi}{4}+\frac{\gamma}{4\pi})}\pi^{-\frac{s}{2}}\Gamma(\frac{s}{2})\zeta(s)=e^{(-abs(\Re(s)-\frac{1}{2})+i\Im(s)\frac{\pi}{4}+\frac{\gamma}{4\pi})}\frac{\xi(s)}{\frac{1}{2}s(s-1)}$ for $0 \le \Re(s) \le 1$ which has finite magnitude ($\lesssim (1+\frac{\gamma}{2})\pi$ for $\Re(s)=\frac{1}{2}$) as $\Im(s) \rightarrow \infty$. 
...
After inspecting the extended Riemann Siegel function behaviour of $\xi(s)$ using (4-6), further multiplicative factors were explicitly trialled to counteract the rapid damping of the $\xi(s)$ to get similar magnitudes more similar to the $\zeta(s)$ behaviour. 

This approach was fruitful and the following normalised Riemann Zeta function $\zeta_{norm}$ was thus defined

\begin{equation}
\zeta_{norm}(s) = e^{(-abs(\Re(s)-\frac{1}{2})+\Im(s)\frac{\pi}{4}+\frac{\gamma}{4\pi})}\pi^{-\frac{s}{2}}\Gamma(\frac{s}{2})\zeta(s) \label{eq:zetanorm}
\end{equation}

which also obeys a simple functional equation

\begin{equation}
\zeta_{norm}(s) = \zeta_{norm}(1-s) \label{eq:normfunc}
\end{equation}
...
\begin{align}
\zeta_{growth factor}(s)&=abs\Big(e^{(-abs(\Re(s)-\frac{1}{2})+\Im(s)\frac{\pi}{4}+\frac{\gamma}{4\pi})}\pi^{-\frac{s}{2}}\Gamma(\frac{s}{2})\Big) \quad \text{for }0 \le \Re(s) \le 1 \label {eq:growth}
\end{align}
...
\begin{equation}
\begin{split}
\zeta_{growth factor}(s) =abs\Big(exp[-abs(\Re(s)-\frac{1}{2})+\Im(s)\frac{\pi}{4}+\frac{\gamma}{4\pi} -\frac{s}{2}log(\pi) \\+\frac{1}{2}log(2\pi)+(\frac{s}{2}-\frac{1}{2})log(\frac{s}{2})-\frac{s}{2}+\frac{1}{12(\frac{s}{2})}- ...])\Big) \quad \text{for }0 \le \Re(s) \le 1 \label {eq:growthseries}
\end{split}
\end{equation}

The fascinating property of the growth factor eqn \eqref{eq:growthseries} is that the real part of $log(\Gamma(\frac{s}{2}))$ is almost the same as $-abs(\Re(s)-\frac{1}{2})+\Im(s)\frac{\pi}{4}+\frac{\gamma}{4\pi} -\frac{s}{2}log(\pi)$ and determines the slow growth of the Riemann Zeta function. The Lindelof hypothesis (1) conjectures that the Riemann Zeta growth factor needs to be $< log(\Im(s))$ for the Riemann Hypothesis to be confirmed.
...
The normalised Riemann Zeta function shares the Riemann Zeta function poles and zeroes and has a finite magnitude behaviour within the critical strip as $\Im(s) \rightarrow \infty$ and can be used to derive an exact growth factor estimate for the magnitude of the Riemann Zeta function. The exact growth factor grows more slowly than $log(\Im(s))$ on the upper critical strip including the critical line confirming the Lindelof Hypothesis. 
