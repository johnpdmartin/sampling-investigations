...
date: "Wednesday, October 7, 2015. fixed eqn 21 Nov 10 2018"
output: pdf_document
---
...
\begin {equation} f_{q-proxy}(b,\theta,n) \propto \exp\{ -\left( \frac{1}{\theta(1-\theta)}b^2 + \frac{-2}{(1-\theta)}b + \frac{1}{(1-\theta)} - 1  \right) \frac{n}{2} \} \,\, \in [0,1] \end{equation}
...
\begin {equation} -(\frac{(b-\mu)^2}{2\sigma_{CLT}^2}) =  -\left( \frac{1}{\theta(1-\theta)}b^2 + \frac{-2}{(1-\theta)}b + \frac{1}{(1-\theta)} - 1  \right) \frac{n}{2}  \end{equation}

first by expanding the LHS of equation (13)

\begin {equation} -(\frac{b^2}{2\sigma_{CLT}^2}+\frac{-2\mu b}{2\sigma_{CLT}^2}+\frac{\mu^2}{2\sigma_{CLT}^2}) =  -\left( \frac{1}{\theta(1-\theta)}b^2 + \frac{-2}{(1-\theta)}b + \frac{1}{(1-\theta)} - 1  \right) \frac{n}{2}  \end{equation}

and then equating coefficients

\begin {equation} \frac{1}{2\sigma_{CLT}^2} \equiv  \frac{n}{2\theta(1-\theta)}  \end{equation}

\begin {equation} \frac{-2\mu}{2\sigma_{CLT}^2} \equiv  \frac{-2n}{2(1-\theta)}  \end{equation}

\begin {equation} \frac{\mu^2}{2\sigma_{CLT}^2} \equiv  \frac{n}{2(1-\theta)} - \frac{n}{2} = \frac{\theta n}{2(1-\theta)}  \end{equation}
...
\begin {equation} f_{q-proxy}(b,\theta,n)_{CLT} \rightarrow \frac{1}{\sqrt{2\pi \theta(1-\theta)n}} \exp\{ \frac{-\left( b-\theta \right)^2}{2\sigma_{CLT}^2} \}  \end{equation}
...
for (i in 1:length(taus)) {
  ...
  quad_densfn[j,i] <- exp(-(grid_int[j]-taus[i])^2/2/q_var[i]^2)
}

quad_densfn[,i] <- quad_densfn[,i]/sum(quad_densfn[,i])
...
samp_perc_cdf <- samp_perc
for (i in 2:n_samp) {
  samp_perc_cdf[i] <- 1/n_samp+samp_perc_cdf[i-1]
}
...
points(y=rep(10/n_grid,n_samp),x=samp_perc_cdf,col="blue",pch="|")
abline(v=taus[1],col="red",lwd=2,lty=3)
...
points(y=rep(10/n_grid,n_samp),x=samp,col="blue",pch="|")
...
points(y=rep(10/n_grid,n_samp),x=samp_perc_cdf,col="blue",pch="|")
abline(v=taus[4],col="red",lwd=2,lty=3)
...
\begin{equation}= \frac{1}{4n(n-1)}\cdot (floor(\frac{n}{2})\cdot2+1)\end{equation} 
