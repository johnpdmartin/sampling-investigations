...
author: "John Martin" 
date: "created:11/01/2018, last modified: 15/01/2018"
output:
  pdf_document:
    fig_crop: no
...
The following two graphs show the low t behaviour and the high t behaviour of $t^{0.1548}$ and $\theta(t)$ based $\zeta_{growth factor}(0.5+it)$ plus some known large Riemann Zeta peaks (scaled by $\pi(1+\gamma/2)$) to represent empirical growth factors if $max(|\zeta_{norm}(0.5+it)|) = \pi(1+\gamma/2)$ is assumed to be true for all t. Since the two estimators provide upper and lower bounds on the growth rate, a simple hybrid product of the two estimators gives a better estimate of the growth. 
...
lines(x=log(10^xs),y=log(sqrt(((10^xs)^0.1548*pi*(1+.57721/2))*(f1g(10^xs)*pi*(1+.57721/2)))),col=6,lty=6)
points(x=log(scaled_peaks_x),y=log(scaled_peaks_h),pch=4,col=2)
legend("topleft", c("t^(0.1548)","Theta'(t) based growth","some scaled known zeta peaks","hybrid sqrt(t^(0.1548)*Theta'(t)) based growth"),
       lty = c(1,1,0,6),cex=.8, lwd = c(2,2,2,2), col = c("blue","green","red","purple"),pch=c(-1,-1,4,-1))
