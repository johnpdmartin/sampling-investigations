...
```{r Zfnzetafunctionoveraninterval_andaveragingitsinversefouriertransform, echo=FALSE, fig.cap="A comparison of the impact of averaging fourier transforms and inverse fourier transforms of different lengths of the partial Euler Product using three truncation lengths (top panel $N_1 = \\sqrt{\\frac{t}{2\\pi}}$, bottom panel $N_1+2000$ and $3N_1$) for the Riemann-Siegel Z function approximation at $\\sigma=0.5+I\\cdot 388858886$.", out.width = c("99%"), fig.show = "hold",  fig.height=4.5, warning=FALSE}
...
lines(1:1000,cumsum(Re(df0$x))/1:1000,col=2,lty=2)
abline(h=0.0004114891482006,col="darkblue",lty=3)
legend(x="right",legend=c("truncation at N1, single inverse fourier transform \n of different length","true value",
    "cumulative mean of inverse fourier transforms \n using different sample lengths"),
       col=c(1,"darkblue",2),lty=c(1,3,2),lwd=2,cex=0.6)
...
lines(1:1000,cumsum(Re(df2000$x))/1:1000,col=6,lty=2)
lines(1:1000,cumsum(Re(df3sqrt$x))/1:1000,col=1,lty=2)
abline(h=0.0004114891482006,col="darkblue",lty=3)
legend(x="topright",legend=c("truncation at N1+2000, single inverse fourier transform \n of different length","truncation at 3N1, single inverse fourier transform \n of different length","true value",
    "cumulative mean of inverse fourier transforms \n using different sample lengths (N1+2000)"
    ,"cumulative mean of inverse fourier transforms \n using different sample lengths (3N1)"),
       col=c(2,3,"darkblue",6,1),lty=c(1,1,3,2,2),lwd=2,cex=0.6)
...
```{r Zfnzetafunctionoveraninterval_andaveragingitsinversefouriertransform_widerspectrum, echo=FALSE, fig.cap="A comparison of the impact of averaging fourier transforms and inverse fourier transforms of different lengths of the partial Euler Product using truncation length $N_1+2000$ based on wider spectrum t=388858886+(-7500,7500) for the Riemann-Siegel Z function approximation at $\\sigma=0.5+I\\cdot 388858886$. The scale in the top panel is the same as figure 3 indicating the wider spectrum can result in more accurate results for truncation length $N_1+2000$. The bottom panel shows the improvement in accuracy with averaging inverse fourier transforms of different length.", out.width = c("99%"), fig.show = "hold",  fig.height=4.5, warning=FALSE}
...
plot(1:1000,Re(df2000$x),typ="b",col=2,ylim=yspan,xlim=c(0,1000),cex=0.3,cex.main=0.9,
     ...
     xlab="number of inverse fourier transforms averaged",
     ylab="Z(0.5+388858886I) value estimate");grid(col="gray")
lines(1:1000,cumsum(Re(df2000$x))/1:1000,col=1,lty=2)
abline(h=0.0004114891482006,col="darkblue",lty=3)
legend(x="topright",legend=c("truncation at N1+2000, single inverse fourier transform \n of different length","true value",
    "cumulative mean of inverse fourier transforms \n using different sample lengths (N1+2000)"),
       col=c(2,"darkblue",1),lty=c(1,3,2),lwd=2,cex=0.6)

plot(1:1000,Re(df2000$x),typ="b",col=2,xlim=c(0,1000),cex=0.3,cex.main=0.9,
     main="impact of taking wider spectrum to analyze when performing \n fourier transforms of different lengths \n and averaging their inverse fourier transforms components \n for partial Euler Product truncation at N1+2000",
     xlab="number of inverse fourier transforms averaged",
     ylab="Z(0.5+388858886I) value estimate");grid(col="gray")
lines(1:1000,cumsum(Re(df2000$x))/1:1000,col=1,lty=2)
...
## Using a finer input grid $\delta t=0.001$

Given the difference between preliminary estimates of the closely spaced Riemann-Siegel Z function non-trivial zeroes and their known values in figure 6. Spectral filtered partial Euler Product based approximation of the Riemann-Siegel Z function with grid size $\delta t=0.001$ is also attempted.


\textbf{example C}; 

approximation 5 - using spectrum collected with grid size $\delta t=0.001$ in the interval t=388858886+(-3832.622,3769.377) based on partial Euler Product truncation at $N_1+2000$ 
...
Finally approximation 6 which uses a wider input spectrum t=388858886+(-7500,7500) for fourier analysis than approximation 5 (t=388858886+(-3832.622,3769.377)) has (i) lower noise closer to example B's behaviour in its cumulative averaging results below and (ii) continues to display in Figure 7 that the spline interpolation preliminary estimates (based on $\delta t=0.001$ grid) of zero crossings remain well aligned with the known non-trivial zeroes values [6,7]. 


approximation 6 - using wider spectrum collected with grid size $\delta t=0.001$ in the interval t=388858886+(-7500,7500) based on partial Euler Product truncation at $N_1+2000$ 

maximum available sample n=15,000,001

below is the inverse fourier transform result for the Riemann-Siegel Z function at $s=0.5+I\cdot 388858886$ under 100 iterations of fourier transforms of different sample lengths

\begin{verbatim}
[1] "index; single run estimate; sample length; cumulative average"
[1] "1 4.11523313832241e-04+7.747e-15i 14802000 4.11523313832241e-04+7.747e-15i"
[1] "2 4.11523361236886e-04-5.037e-15i 14804000 4.11523337534563e-04+1.355e-15i"
[1] "3 4.11523684025954e-04-2.52e-16i 14806000 4.11523453031694e-04+8.19e-16i"
[1] "4 4.11517358095215e-04+1.171e-15i 14808000 4.11521929297574e-04+9.07e-16i"
[1] "5 4.11518825424092e-04-8.59e-16i 14810000 4.11521308522877e-04+5.54e-16i"
[1] "6 4.11515815020734e-04+2.6e-17i 14812000 4.11520392939187e-04+4.66e-16i"
[1] "7 4.11508480722641e-04+9.3e-17i 14814000 4.11518691193966e-04+4.13e-16i"
[1] "8 4.11496329588964e-04+1.232e-15i 14816000 4.11515895993341e-04+5.15e-16i"
[1] "9 4.11482121378464e-04-3.48e-16i 14818000 4.11512143258355e-04+4.19e-16i"
[1] "10 4.11471496624231e-04-2.96e-16i 14820000 4.11508078594942e-04+3.48e-16i"
...
[1] "91 4.11519335612193e-04+3.166e-15i 14982000 4.11488756582388e-04+2.8e-16i"
[1] "92 4.11525330814178e-04-9.941e-15i 14984000 4.11489154128386e-04+1.69e-16i"
[1] "93 4.11529577880114e-04-1.06e-15i 14986000 4.11489588792383e-04+1.56e-16i"
[1] "94 4.11527478420714e-04+5.91e-16i 14988000 4.11489991873536e-04+1.6e-16i"
[1] "95 4.11516275820512e-04+8.3e-17i 14990000 4.11490268546662e-04+1.6e-16i"
[1] "96 4.11506597387029e-04-8.63e-16i 14992000 4.11490438638749e-04+1.49e-16i"
[1] "97 4.11489778760425e-04-1.059e-15i 14994000 4.1149043183588e-04+1.37e-16i"
[1] "98 4.11478113876259e-04+5e-18i 14996000 4.11490306142414e-04+1.35e-16i"
[1] "99 4.11469942953073e-04+1.4821e-14i 14998000 4.11490100453633e-04+2.84e-16i"
[1] "100 4.11463989375082e-04+2.514e-15i 15000000 4.11489839342847e-04+3.06e-16i"
\end{verbatim}

where the reference value for comparison of the Riemann-Siegel Z function at $s=0.5+I*388858886.0)$ is 
$Z(0.5+I*388858886)=0.00041148914820069374095342320372253831362 + 9.442756329679734991 E-33*I$

```{r Zfnzetafunction_interpolationresults_p001_widerinputsprectrum, echo=FALSE, fig.cap="Using an input grid size of $\\delta t=0.001$.  A comparison of the true Riemann-Siegel Z function and (i) the inverse fourier transform approximation at t=388858886.002, 388858886.003 (circle) based on the partial Euler Product using truncation length $N_1+2000$ using spectral filtering, phase conjugate reflection, two imputed imaginary zeroes and averaging inverse fourier transforms of different lengths analyzing the on wider spectrum t=388858886+(-7500,7500) on the critical line around $\\sigma=0.5+I\\cdot 388858886$ and (ii) spline interpolation (red line) fitting the whole grid of inverse fourier transform approximations with grid spacing $\\delta t=0.001$. The top panel shows a closeup of the closely spaced zeroes at t=388858886.0022851... and t=388858886.0023936... [6,7] and the spline interpolation preliminary estimates of the zero crossings. The bottom panel shows the improvement in the inverse fourier transformation approximation accuracy by using averaging of inverse fourier transforms of different length.", out.width = c("99%"), fig.show = "hold",  fig.height=4, warning=FALSE}
...
df2000=read.csv("/home/john/pari/approximation_pm7500input_extra2000_p001_cumulative_mean_estimate.csv")

plot(1:100,Re(df2000$x),typ="b",col=2,xlim=c(0,200),cex=0.3,cex.main=0.9,
     main="impact of taking wider spectrum to analyze when performing \n fourier transforms of different lengths \n and averaging their inverse fourier transforms components \n for partial Euler Product truncation at N1+2000",
     xlab="number of inverse fourier transforms averaged",
     ylab="Z(0.5+388858886I) value estimate");grid(col="gray")
lines(1:100,cumsum(Re(df2000$x))/1:100,col=1,lty=2)
abline(h=0.0004114891482006,col="darkblue",lty=3)
legend(x="topright",legend=c("truncation at N1+2000 single inverse fourier transform \n of different length","true value",
    "cumulative mean of inverse fourier transforms \n using different sample lengths (N1+2000)"),
       col=c(2,"darkblue",1),lty=c(1,3,2),lwd=2,cex=0.6)
